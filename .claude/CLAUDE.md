...
### Review flagged relations (after process/rebuild-graph)
The pipeline auto-generates `entity_groups/relation_review.yaml` with PERSON-PERSON
family/succession relations (SPOUSE_OF, CHILD_OF, HEIR_OF, RELATED_TO) flagged as DRAFT.
- Set `status: CONFIRMED` to keep a relation in the graph
- Set `status: REJECTED` to remove it on next `apply-merges`
- REJECTED entries demote back to DRAFT when new extractions are added (re-review)
...
# Also removes REJECTED relations from relation_review.yaml
```
